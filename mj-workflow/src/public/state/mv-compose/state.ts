...
export function addOutputVideoAsset(store: Store<WorkflowState>, outputUrl: string) {
  ...
  store.update((s) => {
    ...
    if (existing) {
      return { ...s, mediaAssets: mediaAssets.slice(-120), mvVideoAssetId: existing.id };
    }
    ...
    return { ...s, mediaAssets: mediaAssets.slice(-120), mvVideoAssetId: assetId };
  });
}
...
export function buildMvComposePayload(ctx: {
  ...
  const selectedRefIds = Array.isArray(state.selectedReferenceIds)
    ? state.selectedReferenceIds.map((x) => String(x || '').trim()).filter(Boolean).slice(0, 24)
    : [];
  const videoAsset = findAsset(state, state.mvVideoAssetId);
  const audioAsset = findAsset(state, state.mvAudioAssetId);
  const subtitleAsset = findAsset(state, state.mvSubtitleAssetId);
